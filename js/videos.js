...
function atualizaComentario(docRef) {
  ...
  docRef.get().then(document => {
      ...
      for (let i = data.length - 1; i >= 0; i--) {
          ul.innerHTML += '<li style="list-style-type: none; margin-bottom: 10px; display: flex; align-items: center; font-family: \'Inter\', sans-serif;">' + 
                          '<img src='+ data[i].pfp + ' style="border-radius: 50%; width: 40px; height: 40px; margin-right: 10px;"/>' + 
                          '<div style="flex-grow: 1;">' +
                              '<p style="font-size: 1.2em; margin: 0; font-weight: bold;">'+ data[i].username +'</p>'+
                              '<p style="font-size: 0.8em; margin: 0;">'+ data[i].comentario + '</p>' +
                          '</div>' +
                          '</li>';
      }
  });
