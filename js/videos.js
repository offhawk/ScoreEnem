  ...
  firebase
    ...
    .then((querySnapshot) => {
      querySnapshot.forEach((doc) => {
        ...
        watched = userData.watched;
        let isWatched =
          watched.filter((vid) => vid.id == videos[0].id).length > 0;
        if (!isWatched) {
          // Adicione aqui o código SVG para o estado inicial
          olhoSvg.innerHTML =
            '<path d="M15.0007 12C15.0007 13.6569 13.6576 15 12.0007 15C10.3439 15 9.00073 13.6569 9.00073 12C9.00073 10.3431 10.3439 9 12.0007 9C13.6576 9 15.0007 10.3431 15.0007 12Z" stroke="var(--text)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><path d="M12.0012 5C7.52354 5 3.73326 7.94288 2.45898 12C3.73324 16.0571 7.52354 19 12.0012 19C16.4788 19 20.2691 16.0571 21.5434 12C20.2691 7.94291 16.4788 5 12.0012 5Z" stroke="var(--text)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>';
...
function alteraVideo(e) {
  ...
  let isWatched =
    watched.filter((vid) => vid.id == clicado.dataset.id).length > 0;
  if (!isWatched) {
    // Adicione aqui o código SVG para o estado inicial
    olhoSvg.innerHTML =
      '<path d="M15.0007 12C15.0007 13.6569 13.6576 15 12.0007 15C10.3439 15 9.00073 13.6569 9.00073 12C9.00073 10.3431 10.3439 9 12.0007 9C13.6576 9 15.0007 10.3431 15.0007 12Z" stroke="var(--text)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><path d="M12.0012 5C7.52354 5 3.73326 7.94288 2.45898 12C3.73324 16.0571 7.52354 19 12.0012 19C16.4788 19 20.2691 16.0571 21.5434 12C20.2691 7.94291 16.4788 5 12.0012 5Z" stroke="var(--text)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>';
  ...
  const q = firebase
    .firestore()
    .collection("video")
    .where("id", "==", liteEmbedEl.getAttribute("videoid"))
    .get()
    .then((querySnapshot) => {
      querySnapshot.forEach((doc) => {
...
function atualizaComentario(docRef) {
  ...
  docRef.get().then((document) => {
    ...
    for (let i = data.length - 1; i >= 0; i--) {
      ul.innerHTML +=
        "<li style=\"list-style-type: none; margin-bottom: 10px; display: flex; align-items: center; font-family: 'Inter', sans-serif;\">" +
        "<img src=" +
        data[i].pfp +
        ...
        querySnapshot.forEach((doc) => {
          ...
          doc.ref.update({
            liked: firebase.firestore.FieldValue.arrayRemove(
              liteEmbedEl.getAttribute("videoid")
            ),
          });
        });
      });
  ...
  firebase
    ...
    .then((doc) => {
      if (doc.exists) {
        ...
        let isWatched =
          watchedVideos.filter((vid) => vid.id == videoId).length > 0;
        ...
        h1.textContent = isWatched
          ? "Deseja remover este vídeo da lista de assistidos?"
          : "Deseja marcar o vídeo como assistido?";
    ...
    .catch((error) => {
      console.error(
        "Erro ao verificar se o vídeo está marcado como assistido:",
        error
      );
    });
}
...
function marcarAssistido(videoId, isWatched) {
  ...
  if (isWatched) {
    firebase
      ...
      .doc(userId)
      .update({
        watched: firebase.firestore.FieldValue.arrayRemove({
          id: videoId,
          titulo: mainTitle,
        }),
      });
...
function toogleVerMais() {
  let verMaisVisible = false;
let divVerMais = document.querySelector("#videos-container");
  if(!verMaisVisible){
    divVerMais.style.display = "block";
  }else
  divVerMais.style.display = "none";

  verMaisVisible = !verMaisVisible;
}
